...
- **Three.js `remove` / `cleanup`** — `disposeThreeSubtree(node, scene)` in `threeEngine.ts` disposes **`BufferGeometry`** and **`Material`** only when their reference counts under the full **`scene`** equal those under the subtree being removed (GLTF often shares one geometry across several meshes; disposing it once per mesh left siblings with **`attribute.array` undefined** and WebGL **`byteLength`** errors).
